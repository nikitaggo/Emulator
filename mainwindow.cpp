...
void MainWindow::upPressed(bool secPlayer)
{
    ...
    case ZXINTERFACE_IF: if (secPlayer) bus->key_press(3,11); else bus->key_press(1,12);break;
    }
}

void MainWindow::downPressed(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(4, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_DOWN);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_press(2,11); else bus->key_press(2,12);break;
    }
}

void MainWindow::leftPressed(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(4, 11);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_LEFT);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_press(0,11); else bus->key_press(4,12);break;
    }
}

void MainWindow::rightPressed(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(2, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_RIGHT);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_press(1,11); else bus->key_press(3,12);break;
    }
}

void MainWindow::firePressed(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_press(0, 12);break;
    case KEMPSTON_IF: bus->kj_button_press(PORT1F::KJ_FIRE);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_press(4,11); else bus->key_press(0,12);break;
    }
}
...
void MainWindow::upReleased(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(3, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_UP);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_release(3,11); else bus->key_release(1,12);break;
    }
}

void MainWindow::downReleased(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(4, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_DOWN);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_release(2,11); else bus->key_release(2,12);break;
    }
}

void MainWindow::leftReleased(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(4, 11);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_LEFT);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_release(0,11); else bus->key_release(4,12);break;
    }
}

void MainWindow::rightReleased(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(2, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_RIGHT);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_release(1,11); else bus->key_release(3,12);break;
    }
}

void MainWindow::fireReleased(bool secPlayer)
{
    switch (ui->cbJoystickInterface->currentIndex()) {
    case CURSOR_IF: bus->key_release(0, 12);break;
    case KEMPSTON_IF: bus->kj_button_release(PORT1F::KJ_FIRE);break;
    case ZXINTERFACE_IF: if (secPlayer) bus->key_release(4,11); else bus->key_release(0,12);break;
    }
}
...
//    }
    if (event->type() == QEvent::KeyPress) {
        ...
        case W_SCANCODE:   upPressed(true);break;
        case S_SCANCODE: downPressed(true);break;
        case A_SCANCODE: leftPressed(true);break;
        case D_SCANCODE:rightPressed(true);break;
...
//        qDebug() << "Key released: "  << ke->nativeScanCode();
        ...
        case W_SCANCODE:    upReleased(true);break;
        case S_SCANCODE:    downReleased(true);break;
        case A_SCANCODE:    leftReleased(true);break;
        case D_SCANCODE:    rightReleased(true);break;
...
void MainWindow::on_cbPaletteCh_currentIndexChanged(int index)
{
    if (index == 0)
        ui->screen->setPaletteCh(0);
    else
        ui->screen->setPaletteCh(16);
}


void MainWindow::on_actionSave_screensot_triggered()
{
    QString fileName = QFileDialog::getSaveFileName(this, "Select path and name of file", "", "*.scr");
...
void MainWindow::on_action_Load_screenshot_triggered()
{
    QString fileName = QFileDialog::getOpenFileName(this, "Choose u'r destiny", "", "*.scr");
